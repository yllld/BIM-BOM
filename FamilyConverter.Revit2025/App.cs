...
namespace FamilyConverter.Revit2025
{
    public class App : IExternalApplication
    {
        public Result OnStartup(UIControlledApplication application)
        {
            try
            {
                ...
                {
                    // Reusing an existing tab is expected.
                }
                ...
                AddButton(
                    panel,
                    assemblyPath,
                    "FamilyConverterTurboCommand2025",
                    "Turbo\nFreeForm",
                    typeof(TurboCommand),
                    RibbonIconKind.TurboFreeForm,
                    "Сверхбыстрый режим для тяжёлых DWG: Solid сразу создаются как FreeFormElement без предпросмотра и Extrusion.");
            }
            catch (Exception ex)
            {
